...
if (!file_exists(GLPI_CONFIG_DIR . "/config_db.php")) {
   ...
   // Check version
   if ((!isset($CFG_GLPI["version"]) || (trim($CFG_GLPI["version"]) != GLPI_SCHEMA_VERSION))
         ...
         if ($error < 2) {
            ...
            if (!isset($CFG_GLPI["version"])) {
               ...
               if (strlen(GLPI_SCHEMA_VERSION) > 40) {
                  ...
                  if (strlen(trim($CFG_GLPI['version'])) < 40) {
               ...
               } else if (trim($CFG_GLPI["version"]) < GLPI_SCHEMA_VERSION) {
                  $older = true;
               } else if (trim($CFG_GLPI["version"]) > GLPI_SCHEMA_VERSION) {
                  $newer = true;
               }
            ...
            if ($older === true) {
               ...
               if ($dev === true) {
                  echo '<p class="red"><strong>' . __('You are using a development version, be carefull!') . '</strong><br/>';
                  echo "<input type='checkbox' required='required' id='agree_dev'/><label for='agree_dev'>" . __('I know I am using a unstable version.') . "</label></p>";
